...
def verify(hash, passw):
    try:
        ...
        if isinstance(e, VerifyMismatchError) or not isinstance(
            e, InvalidHash
        ):
            return False
        os.remove('password.txt')
...
def closeprocess():
    if request.method == "GET":
        return "Remote Access Software was installed correctly.", 200, {"ContentType": "application/text"}
